      ...
      <div class="flex justify-center my-6">
        <div class="flex border-t flex-col w-full p-8 text-gray-800 bg-white shadow-lg pin-r pin-y md:w-4/5 lg:w-4/5">
          <div class="flex-1">
              ...
              <tbody>
                <tr v-for="item in lineItems" :key="item.id">
                  ...
                  <td>
                    ...
                    <button type="submit" @click="removeFromCart(checkoutID, item.id)" class="text-gray-700 md:ml-4">
            ...
            <a :href="checkoutWebURL" target="_blank">
              <button class="flex justify-center w-full px-10 py-3 mt-6 font-medium text-white uppercase bg-gray-800 rounded-full shadow item-center hover:bg-gray-700 focus:shadow-outline focus:outline-none">
                ...
                <span class="ml-2 mt-5px text-sm md:text-base">Procceed to checkout</span>
              </button>
            </a>
          </div>
        </div>
  ...
  async mounted () {
    if(process.browser){
      ...
      this.$shopify.checkout.fetch(this.checkoutID).then(checkout => {
        ...
        this.checkoutWebURL = checkout.webUrl;
        this.loading = false;
      }); 
    }
  },
  ...
  computed: {
    total: function () {
      ...
      for(let i = 0; i < this.lineItems.length; i++){
        console.log(this.lineItems[i].variant.price);
        sum += (parseFloat(this.lineItems[i].variant.price) * parseFloat(this.lineItems[i].quantity));
      }
